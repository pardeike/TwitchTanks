...
namespace TwitchChatConnect.Client
{
	public class TwitchChatClient : MonoBehaviour
	{
		...
		private TwitchConnectConnect _twitchConnectConfig;

		private bool _isAuthenticated;

		private static readonly string COMMAND_NOTICE = "NOTICE";
		private static readonly string COMMAND_PING = "PING :tmi.twitch.tv";
		private static readonly string COMMAND_PONG = "PONG :tmi.twitch.tv";
		private static readonly string COMMAND_JOIN = "JOIN";
		private static readonly string COMMAND_PART = "PART";
		private static readonly string COMMAND_MESSAGE = "PRIVMSG";
		private static readonly string CUSTOM_REWARD_TEXT = "custom-reward-id";
		private static readonly char BADGES_SEPARATOR = ',';
		private static readonly char BADGE_SEPARATOR = '/';
		...
		private static readonly string LOGIN_FAILED_MESSAGE = "Login authentication failed";

		// For now we compare against the 'success message' + the username. https://dev.twitch.tv/docs/irc/guide
		private static readonly string LOGIN_SUCCESS_MESSAGE = ":tmi.twitch.tv 001";

		// Custom error message when the username does not belong to the given user token, but the user token is valid.
		private static readonly string LOGIN_WRONG_USERNAME =
		...
		private readonly Regex _cheerRegexp = new Regex(@"(?:\s|^)cheer([0-9]+)(?:\s|$)", RegexOptions.IgnoreCase);
		...
		/// Invokes onSuccess when the connection is done, otherwise onError will be invoked.
		/// </summary>
		public void Init(TwitchConnectConnect twitchConnectConfig, OnSuccess onSuccess, OnError onError)
		{
			...
			{
				string errorMessage = "TwitchChatClient.Init :: Twitch connect data is invalid, all fields are mandatory.";
		...
		private void ReadChatLine()
		{
			...
			else if (message.Contains(COMMAND_JOIN))
			{
				...
				_ = TwitchUserManager.AddUser(username);
			}
		...
		{
			if (!IsConnected()) return false;
			_ = StartCoroutine(SendTwitchChatMessageWithDelay(message, seconds));
			return true;
		}
